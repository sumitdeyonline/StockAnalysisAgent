...
from agent_local import setup_agent, is_financial_query
...
st.set_page_config(page_title="Stock Agent (Local Mode)", page_icon="📈", layout="wide")
...
st.title("📈 Stock Analysis Agent (Local Mode)")
st.markdown("This agent has access to real-time market data , recent news, and semantic context. **Currently running on local hardware using the fine-tuned Mistral adapter.**")
...
def __stream_agent_response(user_message, status_placeholder):
    ...
    try:
        ...
        import traceback
        traceback.print_exc()
        yield f"\n\n*Agent stream interrupted: {repr(e)}*"
